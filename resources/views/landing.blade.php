    ...
    <main id="main" class="mx-auto flex w-full max-w-4xl flex-1 flex-col px-5">
        ...
        <header class="py-16 text-center sm:py-24">
            <h1 class="mx-auto max-w-2xl text-balance text-4xl font-bold tracking-tight sm:text-6xl">
                {{ __('Your links.') }}
                <span class="text-brand-600 dark:text-brand-400">{{ __('Your domain.') }}</span>
                {{ __('Your data.') }}
            </h1>
            ...
            <div class="mt-10 flex flex-wrap items-center justify-center gap-4">
                <a href="{{ route('register') }}"
                   class="rounded-full bg-primary px-6 py-3 font-semibold text-primary-fg transition hover:bg-primary-hover focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 motion-reduce:transition-none">
                    {{ __('Create your page — it\'s free') }}
                </a>
                @if ($exampleUsername !== null)
                    <a href="{{ url('/'.$exampleUsername) }}"
                       class="rounded-full border border-control px-6 py-3 font-medium transition hover:bg-surface focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring motion-reduce:transition-none">
            ...
            <p class="mt-6 text-sm text-muted">
                <a href="{{ config('nexo.repository_url') }}" rel="noopener" class="hover:text-ink">
