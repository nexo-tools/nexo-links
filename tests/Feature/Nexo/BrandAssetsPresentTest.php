...
//
// The ecosystem marks come from config/nexo-ecosystem.php — the app-switcher
// renders one <img> per tool, so a missing file is a hole in the switcher.
...
it('renders this tool mark in the header of the pages that carry the chrome', function () {
    /** @var string $mark */
    $mark = config('nexo-ecosystem.tools.'.config('nexo-ecosystem.current').'.mark');

    foreach (['/', route('help'), route('legal.privacy')] as $url) {
        $html = $this->get($url)->assertOk()->getContent();

        expect(str_contains($html, $mark))->toBeTrue("{$url} does not render the tool mark ({$mark}) in its header.");
    }
});
