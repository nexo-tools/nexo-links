...
it('has no raw Tailwind palette classes in views (use the semantic utilities)', function () {
    ...
    $allowed = [
        // The storefront is themed by its owner, not by the visitor: when a custom
        // background is set, ink and cards are picked from that background's own
        // brightness (Page::hasLightBackground), so a token that flips with
        // data-theme would override the owner's design. DarkModeCoverageTest
        // carves out the same file and asserts that logic still exists.
        'pages/show.blade.php' => 'per-page owner theming, ink derived from the chosen background',

        // The scrim is not a surface — it dims whatever is behind it, in both
        // themes alike, so it must NOT flip with the token layer.
        'components/modal.blade.php' => 'modal scrim: deliberately palette-independent',
    ];
