...
it('links the legal pages from the shared footer and lists them in the sitemap', function () {
    // The footer is what makes them reachable from every page; the sitemap is
    // what makes them findable. Both drifted in other tools by simply not
    // being wired after the pages were written.
...
it('reserves the legal paths so no public page can shadow them', function () {
    $reserved = config('nexo.reserved_usernames');

    foreach (['privacidad', 'terminos'] as $segment) {
